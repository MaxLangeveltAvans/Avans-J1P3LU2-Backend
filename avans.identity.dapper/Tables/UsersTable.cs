...
namespace Avans.Identity.Dapper
{
        ...
        where TUserToken : IdentityUserToken<TKey>, new()
    {
        ...
        public virtual async Task<bool> CreateAsync(TUser user) {
            const string sql = @$"
                INSERT INTO {SchemaName}.AspNetUsers
            ...
            using (var transaction = DbConnection.BeginTransaction()) {
                ...
                if (logins?.Count() > 0) {
                    const string deleteLoginsSql = @$"
                        DELETE
                        FROM {SchemaName}.AspNetUserLogins
                        WHERE UserId = @UserId;
                    ";
                    await DbConnection.ExecuteAsync(deleteLoginsSql, new { UserId = user.Id }, transaction);
                    const string insertLoginsSql = @$"
                        INSERT INTO {SchemaName}.AspNetUserLogins (LoginProvider, ProviderKey, ProviderDisplayName, UserId)
        ...
        /// <inheritdoc/>
        public virtual async Task<IEnumerable<TUser>> GetUsersAsync()
        {
            const string sql = @$"
                SELECT *
                FROM {SchemaName}.AspNetUsers AS u;
            ";
            var users = await DbConnection.QueryAsync<TUser>(sql);
            return users;
        }
        ...
        public virtual async Task<IEnumerable<TUser>> GetUsersInRoleAsync(string roleName) {
            const string sql = @$"
                SELECT *
                FROM {SchemaName}.AspNetUsers AS u
                INNER JOIN {SchemaName}.AspNetUserRoles AS ur ON u.Id = ur.UserId
                INNER JOIN {SchemaName}.AspNetRoles AS r ON ur.RoleId = r.Id
        ...
        public virtual async Task<IEnumerable<TUser>> GetUsersForClaimAsync(Claim claim) {
            const string sql = @$"
                SELECT *
                FROM {SchemaName}.AspNetUsers AS u
                INNER JOIN {SchemaName}.AspNetUserClaims AS uc ON u.Id = uc.UserId
